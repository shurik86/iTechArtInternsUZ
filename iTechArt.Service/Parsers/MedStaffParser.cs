        ...
        public async Task ParseCSVAsync(IFormFile file)
        {
            ...
            for (int i = 1; i < csvLines.Length; i++)
            {
                ...
                MedStaffDTO medStaff = new MedStaffDTO()
                {
                    ...
                    DateOfBirth = DateTime.Parse(rowData[5]),
                    Address = rowData[6],
        ...
        public async Task ParseExcelAsync(IFormFile file)
        {
            ...
            for (int row = 2; row <= rowCount; row++)
            {
                var medStaff = new MedStaffDTO()
                {
                    ...
                    DateOfBirth = DateTime.Parse(worksheet.Cells[row, MedStaffIndexConstants.DATEOFBIRTHn].Value.ToString()),
        ...
        public async Task ParseXMLAsync(IFormFile file)
        {
            ...
            for(int node = 0; node < nodes.Count; node++)
            {
                MedStaffDTO medStaff = new MedStaffDTO
                {
                    ...
                    DateOfBirth = DateTime.Parse(nodes[node][MedStaffIndexConstants.DATEOFBIRTH].InnerText),
